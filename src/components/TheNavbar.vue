			...
			<div
				...
				<div id="leftNav" class="self-center flex gap-x-3">
					...
					<div class="flex sm:hidden self-center place-content-center cursor-pointer">
						...
						<SearchModal :show="showSearchModal" v-model="searchWord"
				...
				<div id="rightNav" class="self-center hidden gap-x-3 max-h-8 lg:flex">
					<BaseInput v-model="searchWord" :placeholder="$t('common.search')" style-type="search"
					...
					<HUMenu as="div" class="self-center flex">
						...
						<transition enter-active-class="transition duration-100 ease-out"
							...
							<MenuItems
								...
								<div class="cursor-pointer flex place-items-center max-h-20 mt-8 mb-5"
									@mouseover="() => { hoverOverWTLogo = true }" @mouseleave="() => { hoverOverWTLogo = false }"
									@click="() => { $router.push({ name: 'dashboard' }) }">
									<img class="w-14 h-14 cursor-pointer mr-2" src="/img/logo.svg" />
									<span class="text-xl text-primary pb-2 font-bold tracking-wide">Wisdom<div
											class="text-[color:var(--wt-color-text-lm)] font-bold -mt-1">Treasures</div>
										</span>
									<ArrowRightIcon class="w-6 ml-1 opacity-0"
										:class="[{ 'arrowAppear': hoverOverWTLogo, 'arrowDisAppear': hoverOverWTLogo === false }]" />
								</div>
